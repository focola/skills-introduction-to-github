	...
	#define MAX_DISEASES 100  // 疾病数据库容量
	// 疾病-科室对应结构
	typedef struct {
	    char disease[50];
	    char department[50];
...
case 4: {
    ...
    char *mainDepartments[] = {
        ...
        "外科-骨科", "外科-普外科", "内科-神经内科",  // 新增神经内科
        "外科-神经外科"
    };
    const int deptCount = 7;  // 科室数量调整为7

    // 列宽计算（科室名称和疾病名称取最大长度）
    int colWidths[7] = {0};
    for(int d=0; d<deptCount; d++){
        // 科室名称本身占用的宽度
        int deptNameLen = strlen(mainDepartments[d]) * 2; // 中文字符按2单位计算
    ...
    // 打印分隔线（考虑中文字符宽度）
    for(int d=0; d<deptCount; d++){
        for(int i=0; i<colWidths[d]/2; i++) printf("—"); // 中文破折号
        printf(" ");
    }
    printf("\n");
    ...
    // 收集各科室疾病数据
    struct DeptDiseases {
        char diseases[10][50];  // 每个科室最多10种疾病
        int count;
    } deptData[7] = {0};
    ...
    for(int line=0; line<10; line++){
        for(int d=0; d<deptCount; d++){
            if(line < deptData[d].count){
                printf("%2d.%-*s", 
                    line+1,
                    colWidths[d]-3,  // 调整编号后的间距
                    deptData[d].diseases[line]);
            } else {
                printf("%-*s", colWidths[d], ""); // 空白填充
            }
        }
        printf("\n");
    }
